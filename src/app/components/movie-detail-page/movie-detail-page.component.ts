...
export class MovieDetailPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get(this.url).subscribe(data => {
      ...
      this.name = data[1].original_title;
      this.tagline = data[1].tagline;
      this.overview = data[1].overview;
      this.yearetc.push(data[1].release_date.substring(0, 4), data[1].vote_average, data[1].runtime);
      ...
      for (let i = 0; i < 20 ; i++){
        ...
        this.recTitles.push(data[4][i].title);
        this.recIds.push(data[4][i].id);
        this.recIds[i] = '/watch/movie/'.concat(this.recIds[i]);
        this.recImgs[i] = this.defaulturl.concat(this.recImgs[i]);
        // 相似
        this.simImgs.push(data[5][i].poster_path);
        this.simTitles.push(data[5][i].title);
        this.simIds.push(data[5][i].id);
        this.simIds[i] = '/watch/movie/'.concat(this.simIds[i]);
  ...
  getCastDetail(id){
    ...
    this.http.get('/cast/' + id).subscribe((data) => {
      ...
      // console.log(this.castOtherName);
    });
  }
